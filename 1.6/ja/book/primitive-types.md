...
これは異なる数値型のリストにそれらの標準ライブラリのドキュメントへのリンクを付けたものです。
...
<!--array.-->
配列は`[T; N]`という型を持ちます。
この`T`記法については[ジェネリックのセクションの中][generics]で話すでしょう。
...
`&`はスライスが参照と同じであることを示し、`[]`はレンジを持ち、あなたにスライスの長さを定義させます。
...
<!--[generics][generics].-->
スライスは型`&[T]`を持ちます。
[ジェネリック][generics]をカバーするときにその`T`について話すでしょう。
